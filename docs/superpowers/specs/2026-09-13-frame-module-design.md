...
|---|---|---|
| S1 | Collider residency probe ignores volumes and overrides; after consolidation clears a region's ops, a filled/pasted chunk can probe as surface-free and get no collider | `collider_streamer.cpp:32-43` (verified: generator + ops only); `consolidation.cpp:157` `clear_region_through`; `test_connectivity.gd:34-38` sets `max_override_bricks = 1` — **Sub-project 3 result:** FIXED (`9757819`, passing `test_a_consolidated_fill_in_open_sky_still_gets_a_collider`, `test_a_pasted_volume_in_open_sky_gets_a_collider`). |
| S2 | Island contact probe omits overrides; CPU `extract_island_volume` has no override input while its GPU counterpart uses them | `island_manager.cpp:82-95`; `volume_set.cpp:382,425,431` — **Sub-project 3 result:** FIXED (`d9a41ea`, passing `test_the_contact_probe_reads_a_consolidated_carve`, `test_the_cpu_island_extract_reads_a_consolidated_carve`). |
| S3 | LoD chunks spanning several regions use one region's override table; `& 31` brick wrap may alias edits every 25.6 m; `gather_ops` truncates at 256 | `mesh_service.cpp:826-833`; `field_ops.glslh:249,294`; `lod_system.cpp:41` — **Sub-project 5a result:** S3a region-map brick wrap FIXED (`a799b0c` characterization, `1bd8e1e` fix; `test_a_brick_beside_a_region_border_reads_no_wrapped_override`); S3a single-table jobs FIXED (same commits; `test_a_collider_chunk_lattice_reads_no_wrapped_override`, `test_an_island_lattice_across_a_region_border_reads_no_wrapped_override`); S3b LoD origin table FIXED (same commits; `test_a_lod_chunk_reads_overrides_of_every_region_it_covers`); S3c LoD op prefix FIXED (`dd4fff3` characterization, `d0910b8` fix; `test_an_over_cap_lod_chunk_is_refused_not_truncated`, `test_ops_too_small_for_a_lod_level_do_not_count_against_its_cap`). |
...
| S5 | Menu ranges contradict C++ clamps | `debug_menu.gd:22,38` vs `grass_settings.h:31` and the emissive clamp — **Sub-project 3 result:** FIXED (`0b8b9a8`, passing `test_every_slider_range_contains_the_shipped_value_and_sits_inside_the_clamp`, native row invariants). |
| S6 | `set_quality_tier` discards per-knob tweaks | `orchestrator.cpp:605-609` — **Sub-project 3 result:** FIXED (`aeea5f8`, passing `test_a_tweak_survives_a_tier_change`, `test_effects_turned_off_before_a_tier_stay_off`). |
