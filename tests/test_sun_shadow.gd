...
func test_the_map_is_the_stated_size_and_covers_the_world() -> void:
	...
	var d: Dictionary = w.hooks().debug_sun_shadow_stats(0)
...
func test_the_map_moves_by_whole_texels_as_the_camera_walks(timeout := 120000) -> void:
	...
	w.hooks().debug_sun_shadow_build(0, true)
	var stats: Dictionary = w.hooks().debug_sun_shadow_stats(0)
	...
	for step in [texel * 0.1, texel * 0.9, texel * 6.0, 137.0]:
		var rebuilds: int = w.hooks().debug_sun_shadow_stats(0)["rebuilds"]
		...
		assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).override_failure_message(
			"the forced rebuild at step %.2f m was refused" % step).is_equal(rebuilds + 1)
		var moved := _map_texel(w.hooks().debug_sun_shadow_stats(0)["view_proj"], feature)
		var d := moved - base
...
func test_sub_texel_camera_motion_does_not_rebuild_the_map(timeout := 60000) -> void:
	...
	w.hooks().debug_sun_shadow_build(0, true)
	var texel: float = w.hooks().debug_sun_shadow_stats(0)["texel_world"]
	var before: int = w.hooks().debug_sun_shadow_stats(0)["rebuilds"]
	...
	for i in range(20):
		...
		w.hooks().debug_sun_shadow_build(0, false)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).override_failure_message(
		"sub-texel camera motion rebuilt the shadow map %d times"
		% [int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]) - before]).is_less_equal(before + 1)
	before = w.hooks().debug_sun_shadow_stats(0)["rebuilds"]
	...
	w.hooks().debug_sun_shadow_build(0, false)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).is_equal(before + 1)
...
func test_the_bias_is_in_depth_units_not_metres(timeout := 120000) -> void:
	...
	assert_bool(await settle(w, Vector3(60, 80, 60), Vector3(1, -0.3, 1).normalized())).is_true()
	w.hooks().debug_sun_shadow_build(0, true)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).is_greater(0)
...
func test_a_lazy_rebuild_does_not_fire_every_frame(timeout := 60000) -> void:
	...
	for i in range(11):
		w.hooks().debug_sun_shadow_build(0, false)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).is_equal(0)
	w.hooks().debug_sun_shadow_build(0, false)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).is_equal(1)
...
func test_a_moved_sun_rebuilds_without_waiting_for_the_throttle(timeout := 60000) -> void:
	...
	w.hooks().debug_sun_shadow_build(0, true)
	var before: int = w.hooks().debug_sun_shadow_stats(0)["rebuilds"]
	var matrix_before: PackedFloat32Array = w.hooks().debug_sun_shadow_stats(0)["view_proj"]
	# A clean pass refuses an unforced build...
	w.hooks().debug_sun_shadow_build(0, false)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).is_equal(before)
	# ...but not when the sun has moved.
	light.rotation = Vector3(-0.5, 1.4, 0.0)
	await get_tree().process_frame
	w.hooks().debug_sun_shadow_build(0, false)
	assert_int(w.hooks().debug_sun_shadow_stats(0)["rebuilds"]).is_equal(before + 1)
	var matrix_after: PackedFloat32Array = w.hooks().debug_sun_shadow_stats(0)["view_proj"]
	var moved := false
	for i in range(16):
...
func test_moving_the_sun_moves_the_shadow(timeout := 60000) -> void:
	...
	var low_matrix: PackedFloat32Array = w.hooks().debug_sun_shadow_stats(0)["view_proj"]

	light.rotation = Vector3(-1.2, 2.4, 0.0) # high sun, opposite azimuth
	await get_tree().process_frame
	w.hooks().debug_sun_shadow_build(0, true)
	var high: Array = _shadow_mask(w)
	var high_matrix: PackedFloat32Array = w.hooks().debug_sun_shadow_stats(0)["view_proj"]
