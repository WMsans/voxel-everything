...
func test_the_map_is_the_stated_size_and_covers_the_world() -> void:
	var w := make_world()
	var d: Dictionary = w.debug_sun_shadow_stats()
...
func test_a_lazy_rebuild_does_not_fire_every_frame(timeout := 60000) -> void:
	...
	assert_int(w.debug_sun_shadow_stats()["rebuilds"]).is_equal(0)
	w.debug_sun_shadow_build(false)
	assert_int(w.debug_sun_shadow_stats()["rebuilds"]).is_equal(1)
