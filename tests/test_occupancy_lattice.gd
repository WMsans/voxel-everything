...
func test_thin_carve_between_probe_samples_is_solid_in_the_occupancy_grid(timeout := 60000) -> void:
	...
	w.debug_apply_sphere_subtract(c, 0.25) # nearest 0.4 m probe sample is 0.346 m away
	w.debug_stream_region(Vector3i(0, 1, 0))
	for i in range(16):
	...
	assert_int(state).is_equal(w.debug_cell_state(cell))
...
func test_no_surface_fallback_matches_cell_state_probe(timeout := 60000) -> void:
	var w := make_world()
	var d: Dictionary = w.debug_occupancy_fallback_diff(Vector3i(0, 2, 0))
