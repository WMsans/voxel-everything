...
func test_the_gbuffer_allocates_at_the_requested_size() -> void:
	var w := make_world()
	var d: Dictionary = w.hooks().debug_gbuffer_stats(320, 180)
...
func test_the_history_is_half_resolution_and_rounds_up_from_odd_sizes() -> void:
	var w := make_world()
	var d: Dictionary = w.hooks().debug_gbuffer_stats(321, 181)
	assert_int(d["half_width"]).is_equal(160)
	assert_int(d["half_height"]).is_equal(90)
	var tiny: Dictionary = w.hooks().debug_gbuffer_stats(1, 1)
...
func test_re_ensuring_at_the_same_size_reuses_the_same_textures() -> void:
	var w := make_world()
	var a: Dictionary = w.hooks().debug_gbuffer_stats(256, 144)
	var b: Dictionary = w.hooks().debug_gbuffer_stats(256, 144)
...
func test_a_different_size_reallocates() -> void:
	var w := make_world()
	var a: Dictionary = w.hooks().debug_gbuffer_stats(256, 144)
	var b: Dictionary = w.hooks().debug_gbuffer_stats(512, 288)
...
func test_a_degenerate_size_is_refused_rather_than_allocated() -> void:
	var w := make_world()
	var d: Dictionary = w.hooks().debug_gbuffer_stats(0, 180)
