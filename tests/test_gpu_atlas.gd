...
func test_texture_extents_follow_the_per_brick_strides() -> void:
	...
	assert_int(rd.texture_get_data(w.hooks().debug_sdf_atlas(), 0).size()).is_equal(136 * 136 * 136)
	# Material: 16 cells per brick per axis, R8.
	assert_int(rd.texture_get_data(w.hooks().debug_mat_atlas(), 0).size()).is_equal(128 * 128 * 128)
	# Min-max levels: 2 / 4 / 8 cells per brick per axis, RG8 -> two bytes per texel.
	assert_int(rd.texture_get_data(w.hooks().debug_mip_atlas(0), 0).size()).is_equal(16 * 16 * 16 * 2)
	assert_int(rd.texture_get_data(w.hooks().debug_mip_atlas(1), 0).size()).is_equal(32 * 32 * 32 * 2)
	assert_int(rd.texture_get_data(w.hooks().debug_mip_atlas(2), 0).size()).is_equal(64 * 64 * 64 * 2)
...
func test_region_map_starts_empty_and_takes_single_entry_writes() -> void:
	...
	var after := rd.buffer_get_data(w.hooks().debug_region_map()).to_int32_array()
...
func test_region_tables_start_absent() -> void:
	...
	var slice := rd.buffer_get_data(w.hooks().debug_region_tables(), 0, 32768 * 4).to_int32_array()
...
func test_free_list_is_a_full_permutation_of_the_slots() -> void:
	...
	assert_bool(w.hooks().debug_init_atlas()).is_true()
	var rd := w.hooks().debug_local_rd() as RenderingDevice
	var fl := rd.buffer_get_data(w.hooks().debug_free_list()).to_int32_array()
...
func test_frame_counters_reset_clears_the_job_count_but_not_the_overflow_bits() -> void:
	...
	assert_bool(w.hooks().debug_init_atlas()).is_true()
	var rd := w.hooks().debug_local_rd() as RenderingDevice
	rd.buffer_update(w.hooks().debug_frame_counters(), 0, 8,
		PackedInt32Array([7, 3]).to_byte_array())
	assert_int(w.hooks().debug_atlas_stats()["job_count"]).is_equal(7)
...
func test_region_ops_upload_byte_for_byte() -> void:
	...
	var got := rd.buffer_get_data(w.hooks().debug_op_pool(), 2 * 256 * 32, 32)
	assert_array(Array(got)).is_equal(Array(b.data_array))
	var counts := rd.buffer_get_data(w.hooks().debug_op_counts()).to_int32_array()
...
func test_teardown_is_idempotent_and_survives_re_init() -> void:
	...
	w.hooks().debug_teardown_atlas()
	assert_bool(w.hooks().debug_init_atlas()).is_true()
	assert_int(w.hooks().debug_atlas_stats()["free_slots"]).is_equal(512)
