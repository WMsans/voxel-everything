...
func test_subtract_across_a_material_seam_matches() -> void:
	# The stored subtract radius is already effective. Painting rock across part of its
	# volume catches either evaluator reintroducing a per-sample material lookup: one side
	# would change shape and CPU/GPU values would diverge.
