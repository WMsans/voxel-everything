...
func test_full_pool_refusal_preserves_edit_log() -> void:
	var w := make_world()
	w.debug_apply_sphere_subtract(Vector3(12.8, 51.2, 12.8), 100.0)
...
func test_failed_publication_rolls_back_both_gpu_pools() -> void:
	var w := make_world()
	w.debug_apply_sphere_subtract(Vector3(24.4, 51.4, 24.4), 2.0)
	assert_bool(w.debug_consolidate_region(Vector3i(0, 2, 0))).is_true()
	w.debug_apply_sphere_subtract(Vector3(27.0, 51.4, 24.4), 1.5)
