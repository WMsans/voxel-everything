...
func build_pillar(w: VoxelWorld, t: VoxelEditTool, x := PILLAR_X, z := PILLAR_Z) -> void:
	...
	# The material is incidental: a removal's radius comes from the material named at the
	# call site, not from whatever the sphere happens to pass through, so this suite's
	# fixed geometry holds whatever the pillar is made of.
	for i in range(5):
...
func test_a_dense_sphere_cut_pillar_top_falls_clear_of_the_cut(timeout := 180000) -> void:
	...
	var t := tool_of(w)
	for i in range(8):
		# See build_pillar: the material is incidental to what this fixture measures.
