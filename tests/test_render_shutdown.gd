...
func test_shutdown_drains_pending_hiz_readback_before_freeing_resources() -> void:
	...
	var result: Dictionary = world.hooks().debug_hiz_shutdown_probe()
...
func test_render_teardown_invalidates_all_normal_pool_rids_and_reinit_resets_tables() -> void:
	...
	var before: Dictionary = world.hooks().debug_stored_normal_stats()
	_assert_normal_invariants(before)
	assert_int(before["normal_capacity_bytes"]).is_equal(33554432)
	var pool: Dictionary = world.hooks().debug_normal_pool_state()
	...
	assert_int(world.hooks().debug_normal_upload_override(0, _packed_normals(1))).is_greater_equal(0)
	var published: Dictionary = world.hooks().debug_normal_pool_state()
	assert_bool(published["override_offsets_all_minus_one"]).is_false()
	assert_bool(published["volume_offsets_all_minus_one"]).is_true()

	world.hooks().debug_teardown_atlas()
	var torn: Dictionary = world.hooks().debug_normal_pool_state()
	...
	assert_bool(world.hooks().debug_stored_normal_stats().is_empty()).is_true()

	# Reinitialization restores the offset tables to -1: fresh buffers, no spans.
	assert_bool(world.hooks().debug_init_atlas()).is_true()
	var reinit: Dictionary = world.hooks().debug_normal_pool_state()
	...
	var after: Dictionary = world.hooks().debug_stored_normal_stats()
...
func test_physics_teardown_preserves_the_normal_pool_and_its_invariants(timeout := 60000) -> void:
	...
	for i in range(60):
		world.hooks().debug_stream_frame(Vector3(20.0, 56.0, 20.0))
	var d: Dictionary = world.hooks().debug_place_test_island(0, Vector3i(25, 58, 25), Vector3i(26, 59, 26),
	...
	world.hooks().debug_teardown_physics()
	var after: Dictionary = world.hooks().debug_stored_normal_stats()
	_assert_normal_invariants(after)
	...
	assert_bool(world.hooks().debug_init_physics()).is_true()
	var reinit: Dictionary = world.hooks().debug_stored_normal_stats()
