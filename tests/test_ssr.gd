...
func make_world() -> VoxelWorld:
	...
	var quiet := 0
	for i in range(400):
		quiet = quiet + 1 if w.hooks().debug_stream_frame(Vector3(20, 56.2, 20)) == 0 else 0
		if quiet >= 6:
			break
	return w

func test_the_reflection_target_is_half_resolution() -> void:
	var d: Dictionary = make_world().hooks().debug_ssr_probe(0, 321, 181)
...
func test_a_post_opaque_only_blocker_is_reflected() -> void:
	var d: Dictionary = make_world().hooks().debug_ssr_probe(0, 128, 128)
...
func test_medium_uses_twelve_steps_and_off_dispatches_nothing() -> void:
	var w := make_world()
	w.quality_tier = 2
	assert_int(w.hooks().debug_ssr_probe(0, 128, 128)["steps"]).is_equal(12)
	w.quality_tier = 0
	var off: Dictionary = w.hooks().debug_ssr_probe(0, 128, 128)
...
func test_uncalibrated_normal_roughness_does_not_enable_dynamic_receivers() -> void:
	var w := make_world()
	var alpha_zero: Dictionary = w.hooks().debug_ssr_probe(2, 128, 128)
	var alpha_one: Dictionary = w.hooks().debug_ssr_probe(3, 128, 128)
...
func test_true_sdf_reflections_are_high_only_and_change_only_albedo() -> void:
	...
	w.quality_tier = 2
	var medium: Dictionary = w.hooks().debug_glossy_sdf_probe(origin, dir)
	w.quality_tier = 3
	var high: Dictionary = w.hooks().debug_glossy_sdf_probe(origin, dir)
